...
static ProviderU load_model(const std::string& weights, int64_t& BD, int64_t& DWF) {
  // BD (base depth) / DWF (depthwise) define the arch; without them the forward consumes the
  // wrong layers and emits garbage/NaN. A failed open would leave BD=0 (C++11 sets extraction
  // targets to 0 on failure) -> silent breakage, so fail LOUDLY instead.
  { std::ifstream f(DU + "io.txt");
    if (!f) { printf("cannot open %sio.txt (arch descriptor: '<imgsz> <BD> <DWF>').\n"
                     "  It is generated per size; a fresh checkout may lack it. For yolox_tiny:\n"
                     "    printf '96 1 0\\n' > %sio.txt\n", DU.c_str(), DU.c_str()); std::exit(1); }
    int64_t im; f >> im >> BD >> DWF;
    if (BD < 1) { printf("bad BD=%lld in %sio.txt (expected >=1)\n", (long long)BD, DU.c_str()); std::exit(1); } }
...
static int cmd_detect(const Args& a) {
  ...
  std::vector<int64_t> strides = {8,16,32};
  prov.i=0; auto raw = yolox_forward_unfused(x, prov, false, BD, (bool)DWF);
  if (a.geti("debug", 0)) {                                   // per-level raw head stats (NaN/Inf hunt)
    const char* lvn[3] = {"P3/8","P4/16","P5/32"};
    for (size_t L = 0; L < raw.size(); ++L) { double mn=1e300,mx=-1e300; size_t nan=0,inf=0;
      for (float v : raw[L]->data) { if (std::isnan(v)){++nan;continue;} if (std::isinf(v)){++inf;continue;} if(v<mn)mn=v; if(v>mx)mx=v; }
      printf("[debug] raw %-6s shape=%lldx%lldx%lldx%lld  min=%.3g max=%.3g  NaN=%zu Inf=%zu\n",
             lvn[L<3?L:0], (long long)raw[L]->shape[0],(long long)raw[L]->shape[1],(long long)raw[L]->shape[2],(long long)raw[L]->shape[3], mn,mx,nan,inf); }
  }
